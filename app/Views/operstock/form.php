...
<head>
    ...
    <title>Form Operstock - Perpindahan Stok</title>
        ...
        <div class="form-card">
            <div class="form-header">
                <h1>
                    ...
                    Form Operstock - Perpindahan Stok
                </h1>
            </div>
                ...
                <form id="formOperstock">
                    ...
                    <div class="section-card">
                        ...
                        <div class="form-grid">
                            ...
                            <div class="form-group">
                                <label for="gudang_asal">
                                    <i class="fas fa-warehouse"></i>
                                    Gudang Asal
                                </label>
                                <select name="gudang_asal" id="gudang_asal" required>
                        ...
                        <div class="history-section" id="transferHistory">
                            <h5 style="color: #3b82f6; font-weight: 600; margin-bottom: 10px;">
                                <i class="fas fa-history"></i> Riwayat Perpindahan Terakhir
                            </h5>
        ...
        $(document).ready(function() {
                    ...
                    <div class="item-row" data-index="${index}">
                        <div class="item-grid">
                            <div class="form-group">
                                ...
                                <div class="stock-info asal-stock"></div>
                                <div class="stock-info tujuan-stock"></div>
                            </div>
                            <div class="form-group">
            ...
            function loadTransferHistory() {
                ...
                if (gudangAsal && gudangTujuan && gudangAsal !== gudangTujuan) {
                    $.getJSON('<?= base_url('operstock/get-transfer-history') ?>', {
                            ...
                            history.forEach(function(item) {
                                historyHtml += `
                                    <div class="history-item">
                                        <strong>${item.no_surat_jalan}</strong> - ${item.waktu_kirim}<br>
                                        <small>${item.gudang_asal} → ${item.gudang_tujuan} (${item.total_items} items)</small>
                                    </div>
                                `;
                            });
            ...
            function updateStockInfo(row) {
                ...
                if (gudangAsal && gudangTujuan && produk) {
                    const asalStock = row.find('.asal-stock');
                    const tujuanStock = row.find('.tujuan-stock');
                    
                    asalStock.removeClass('available warning').addClass('loading');
                    asalStock.html('<i class="fas fa-spinner fa-spin"></i> Mengecek stok asal...');
                    
                    tujuanStock.removeClass('available warning').addClass('loading');
                    tujuanStock.html('<i class="fas fa-spinner fa-spin"></i> Mengecek stok tujuan...');
                    
                    $.getJSON('<?= base_url('operstock/get-both-stocks') ?>', {
                        id_gudang_asal: gudangAsal,
                        id_gudang_tujuan: gudangTujuan,
                        id_produk: produk
                    })
                    .done(function(data) {
                        asalStock.removeClass('loading').addClass('available');
                        asalStock.html(`<i class="fas fa-warehouse"></i> Stok Asal: ${data.asal.jumlah_dus} Dus, ${data.asal.jumlah_satuan} Satuan`);
                        
                        tujuanStock.removeClass('loading').addClass('available');
                        tujuanStock.html(`<i class="fas fa-warehouse"></i> Stok Tujuan: ${data.tujuan.jumlah_dus} Dus, ${data.tujuan.jumlah_satuan} Satuan`);
                    })
                    .fail(function() {
                        asalStock.removeClass('loading').addClass('warning');
                        asalStock.html('<i class="fas fa-times-circle"></i> Gagal cek stok asal');
                        
                        tujuanStock.removeClass('loading').addClass('warning');
                        tujuanStock.html('<i class="fas fa-times-circle"></i> Gagal cek stok tujuan');
                    });
                }
            }
            ...
            $('#gudang_asal, #gudang_tujuan').on('change', function() {
                loadTransferHistory();
            ...
            $('#formOperstock').on('submit', function(e) {
                ...
                $.ajax({
                    ...
                    complete: function(){
                        btn.html('<i class="fas fa-save"></i> Simpan Perpindahan Stok').prop('disabled', false);
                    }
                });
            });
